...
class ActivityController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $res = (new Query())->select('MONTH(activity.date) as month_num, MONTHNAME(activity.date) as month, count(YEAR(activity.date)) as visits')->from('activity')->groupBy('month, month_num')->orderBy('month_num')->limit(12)->all();
        /*$data = "['Month','visits'],";
        foreach ($res as $item) {
